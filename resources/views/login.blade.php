<x-layouts>
    <x-navbar />
    <main class="  flex-1 mt-15  md:mt-20">
        <div class="items-center flex  pt-5 lg:space-x-8 mx-auto px-6 md:px-10 lg:max-w-6xl">
            <img class="hidden  w-1/2 lg:flex" src="{{ asset('assets/undraw_developer_activity_re_39tg.svg') }}"
                alt="">
            <div class="w-full lg:w-1/2 ">
                ...
                <form action="/login" method="post">
                    @csrf
                    <div class="mt-4 ">
                        <label class="block mb-2 text-sm text-gray-600" for="">E-Mail</label>
                        <input
                            ...
                            type="email" name="email" value="{{ Cookie::get('emailCookie') != Null ? Cookie::get('emailCookie') : ''}}" required autofocus>
                            @error('email')
                    ...
                    <div class="mt-4">
                        <label class="block mb-2 text-sm text-gray-600" for="">Password</label>
                        <input
                            class="w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg focus:border-purple-500 focus:outline-none "
                            type="password" name="password" value="{{ Cookie::get('passCookie') != Null ? Cookie::get('passCookie') : ''}}" required>
                        @error('password')
                        <div class="text-xs text-red-500">
                            {{ $message }}
                        </div>                  
                    ...
                    <div class="mt-4">
                        <input type="checkbox" name="remember" value="1" id="remember"
                            class="border-gray-300 rounded text-primary checked:bg-purple-500  focus:border-indigo- focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="remember">
                        <label for="remember" class="ml-2 text-gray-700" for="remember">Remember Me</label>
                    </div>
                    <div class="mt-8">
                        <button type="submit"
                            class=" px-4 py-2 rounded-md text-sm w-full font-medium border-0 focus:outline-none focus:ring transition-colors text-white bg-purple-500 hover:bg-purple-600 active:bg-purple-700 focus:ring-purple-300">Login</button>
                    </div>
                    <div class="mt-4 flex align-middle justify-center">
                        <hr class="w-1/3 border-gray-800 mt-3">
                        <div class="text-gray-600 mx-2">Or Login with</div>
                        <hr class="w-1/3 border-gray-800 mt-3">
                    </div>
                    <div class="mt-4 flex justify-center gap-3">
                        <a href="/sign-in/google" class="" >
                ...
                <div class="mt-5">
                    <p class="text-gray-600">Don't have an account? <a href="/register"
                            class="text-purple-500">Register here</a></p>
                </div>
            </div>
        </div>
